...
import { printTotemTicket } from '@/lib/totem-print-ticket';

// Lazy-load all wizard modals — they only mount when the user opens them.
...
// ── Feature Banner Fallback Gradients (sem URLs externas para evitar flash de imagens "erradas") ──
...
function TotemKiosk() {
  ...
  const { data: customBannerImages = {}, isFetched: bannersFetched } = useQuery({
  ...
  // Totem esconde produtos físicos zerados (estoque <= 0).
  // Bebidas feitas na hora (is_prepared / infinite) continuam visíveis.
  const activeProducts = useMemo(() =>
    products.filter(p => {
      if (p.isActive === false) return false;
      const category = rawCategories.find(c => c.id === p.categoryId);
      if (!isInfiniteStockProduct(p, category?.name) && (p.stock ?? 0) <= 0) return false;
      return true;
    }),
  ...
  const getFeatureImage = (id: string): string | null => {
  ...
  return (
    <div className="h-full w-full bg-white flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 bg-white shadow-lg border-b border-[hsl(263,15%,85%)] px-3 pt-3 pb-3 space-y-1.5">
          ...
          <div className="flex justify-center gap-1">
            ...
            <button onClick={() => { setSearchQuery(''); setShowKeyboard(false); }}
              className="w-20 h-12 rounded-xl bg-destructive text-white font-bold text-sm flex items-center justify-center gap-1">
            ...
            <div className="flex gap-3 overflow-x-auto totem-horizontal-scroll pb-1 px-2">
              {!bannersFetched
                ? featureBanners.map(fb => (
                    <div
                      key={fb.id}
                      className="flex-shrink-0 w-[300px] h-[180px] rounded-2xl overflow-hidden relative snap-start shadow-lg bg-gradient-to-br from-muted to-muted/50 animate-pulse"
                    />
                  ))
                : featureBanners.map(fb => {
                    ...
                    return (
                      <button key={fb.id} onClick={fb.onClick}
                        ...
                        {imageUrl ? (
                          ...
                          <>
                            <div className="absolute inset-0" style={{ background: FEATURE_FALLBACK_GRADIENTS[fb.id] || 'linear-gradient(135deg, hsl(263,78%,50%), hsl(280,70%,40%))' }} />
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-white px-4 text-center">
                              <FbIcon className="h-12 w-12 mb-2 drop-shadow-lg" />
                              <span className="text-lg font-black uppercase tracking-wide drop-shadow">{fb.title}</span>
                              <span className="text-xs font-medium opacity-90 mt-1 drop-shadow">{fb.desc}</span>
                            </div>
                          </>
                        )}
                      </button>
        ...
        {!isSearchActive && (
          <div className="px-3 pb-2">
            ...
            <div className="grid grid-cols-4 gap-2 px-0">
              ...
              {/* Botão Carrinho — sempre evidente, mesmo tamanho dos cards de drink */}
              <button
                onClick={() => cartCount > 0 && setShowCart(true)}
                disabled={cartCount === 0}
                aria-label="Abrir carrinho"
                className="relative active:scale-95 transition-transform disabled:active:scale-100"
              >
                <div className={`w-full aspect-square rounded-xl overflow-hidden shadow-xl border-[3px] relative flex flex-col items-center justify-center bg-gradient-to-br from-red-500 via-red-600 to-red-700 border-white ${
                  cartCount === 0 ? 'opacity-90' : 'animate-pulse'
                }`}
                style={{ boxShadow: '0 0 24px rgba(239, 68, 68, 0.7), 0 4px 16px rgba(0,0,0,0.35)' }}
                >
                  <ShoppingCart className="w-10 h-10 text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" strokeWidth={2.5} />
                  <span className="mt-1 text-[11px] font-black uppercase tracking-wider text-white drop-shadow">
                    Carrinho
                  </span>
                </div>
                {cartCount > 0 && (
      ...
      <Suspense fallback={null}>
        ...
        {caipiIceOpen && <CaipiIceModal open={caipiIceOpen} onOpenChange={setCaipiIceOpen} onAddCustomDrink={handleAddCustomDrink} />}
      </Suspense>
...
export default function TotemPanel() {
  const { isAuthenticated, role, login, isHydrated } = useAuth();
  ...
  useEffect(() => {
    if (!isHydrated || didAutoLogin.current) return;

    const isStaff = isAuthenticated && (role === 'pdv' || role === 'admin');
    if (isStaff) {
      setLoggedIn(true);
    } else {
      // Auto-login totem — no password required on dedicated hardware
      didAutoLogin.current = true;
      login(
        { id: 'totem-kiosk', name: 'TOTEM', whatsapp: '', role: 'pdv' },
        'pdv'
      );
      setLoggedIn(true);
    }
  }, [isHydrated, isAuthenticated, role, login]);

  if (!loggedIn) {
    return (
      <div className="h-full w-full flex items-center justify-center bg-gradient-to-br from-[hsl(263,78%,12%)] via-[hsl(263,78%,20%)] to-[hsl(280,60%,15%)]">
        <div className="text-center space-y-4">
          <img src={totemLogo} alt="VM" className="h-28 w-auto mx-auto" />
          <p className="text-white/60 text-lg">Iniciando totem...</p>
        </div>
      </div>
    );
  }
