...
BEGIN
    ...
    -- Format: sha256:salt:hash (register_customer hashes as password+salt)
    IF stored_password LIKE 'sha256:%' THEN
        hash_parts := string_to_array(stored_password, ':');
        IF array_length(hash_parts, 1) = 3 THEN
            salt := hash_parts[2];
            computed_hash := encode(extensions.digest((p_password || salt)::bytea, 'sha256'), 'hex');
            RETURN computed_hash = hash_parts[3];
        END IF;
    -- Format: salt:hash (hash_password hashes as salt+password)
