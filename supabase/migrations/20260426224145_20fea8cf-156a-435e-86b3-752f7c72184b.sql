...
BEGIN
  ...
  -- 3) Apagar comprovantes de pagamento
  DELETE FROM public.payment_confirmations WHERE order_id = p_order_id;

  -- 4) Soltar referências em audit/logs (sem FK CASCADE)
  UPDATE public.pix_payment_audit SET order_id = NULL WHERE order_id = p_order_id;

  -- order_items / motoboy_locations / ifood_test_events_log já têm CASCADE/SET NULL
