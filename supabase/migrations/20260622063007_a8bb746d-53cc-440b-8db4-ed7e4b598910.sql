CREATE INDEX IF NOT EXISTS idx_order_items_order_id_kde
ON public.order_items (order_id)
INCLUDE (product_id, is_wizard_item, product_name);

CREATE INDEX IF NOT EXISTS idx_orders_kde_active_recent
ON public.orders (created_at DESC, status)
WHERE status IN ('accepted', 'preparing', 'ready', 'dispatched');
...
BEGIN
  ...
  WHERE o.status IN ('accepted', 'preparing', 'ready', 'dispatched')
    ...
    AND NOT (
      o.external_origin = 'ifood_test'
      OR lower(COALESCE(o.salesperson, '')) IN ('ifood', 'rappi', '99food', 'keeta')
    )
    AND EXISTS (
      SELECT 1
      FROM public.order_items oi
...
REVOKE EXECUTE ON FUNCTION public.get_kitchen_orders_with_items() FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_kitchen_orders_with_items() FROM anon;
