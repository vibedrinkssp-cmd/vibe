...
CREATE OR REPLACE FUNCTION public.customer_login_rpc(
  p_whatsapp TEXT, 
  p_password TEXT
)
RETURNS JSONB
...
-- Also create a check-phone RPC to avoid RLS issues
CREATE OR REPLACE FUNCTION public.check_phone_rpc(p_whatsapp TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_role TEXT;
BEGIN
  SELECT id, name INTO v_user FROM users WHERE whatsapp = p_whatsapp;
  IF NOT FOUND THEN
