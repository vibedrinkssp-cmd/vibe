...
BEGIN
  ...
  -- Validar cálculo do total com tolerância generosa (R$1.00)
  v_calculated_total := p_subtotal + COALESCE(p_delivery_fee, 0) - COALESCE(p_discount, 0);
  IF ABS(v_calculated_total - p_total) > 1.00 THEN
    RAISE LOG 'create_counter_order MISMATCH: subtotal=%, fee=%, discount=%, total=%, calc=%',
      p_subtotal, p_delivery_fee, p_discount, p_total, v_calculated_total;
    RAISE EXCEPTION 'Cálculo do total inconsistente: esperado=%, recebido=%', v_calculated_total, p_total;
  END IF;
  ...
  VALUES (
    p_user_id, 'counter', 'accepted', p_subtotal, COALESCE(p_delivery_fee, 0), COALESCE(p_discount, 0), p_total,
