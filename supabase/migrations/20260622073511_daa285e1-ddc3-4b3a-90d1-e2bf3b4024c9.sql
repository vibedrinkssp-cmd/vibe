CREATE OR REPLACE FUNCTION public.get_kitchen_orders_with_items()
 RETURNS TABLE(id uuid, user_id uuid, address_id uuid, order_type order_type, status order_status, subtotal numeric, delivery_fee numeric, original_delivery_fee numeric, delivery_fee_adjusted boolean, delivery_fee_adjusted_at timestamp with time zone, delivery_distance numeric, discount numeric, total numeric, payment_method payment_method, change_for numeric, notes text, customer_name text, salesperson text, motoboy_id uuid, created_at timestamp with time zone, accepted_at timestamp with time zone, preparing_at timestamp with time zone, ready_at timestamp with time zone, dispatched_at timestamp with time zone, picked_up_at timestamp with time zone, arrived_at timestamp with time zone, delivered_at timestamp with time zone, payment_confirmed boolean, payment_confirmed_at timestamp with time zone, payment_confirmed_by text, external_origin text, external_order_id text, mp_payment_id text, customer_resolved_name text, customer_whatsapp text, address_payload jsonb, items_payload jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
        ...
        AND (
          ...
          OR (
            ...
            AND (
              ...
              OR oi.product_name ILIKE '%copa de%'
              ...
              OR oi.product_name ILIKE '%batida de%'
              OR oi.product_name ILIKE '%monte seu%'
              OR oi.product_name ILIKE '%monte sua%'
              OR oi.product_name ILIKE '%combo natural%'
              OR oi.product_name ILIKE '%combo salgado%'
              OR oi.product_name ILIKE '%combo hamburguer%'
              OR oi.product_name ILIKE '%combo hambúrguer%'
              OR oi.product_name ILIKE '%lanche natural%'
              OR oi.product_name ILIKE '%almoço natural%'
              OR oi.product_name ILIKE '%almoco natural%'
            )
          )
        )
    )
