...
BEGIN
  -- valida pedido
  SELECT id, external_origin, salesperson, status
    INTO v_order
    FROM public.orders
   WHERE id = p_order_id
   FOR UPDATE;
  ...
  -- aceita pedido externo identificado por external_origin OU por salesperson de plataforma
  IF NOT (
       coalesce(v_order.external_origin, '') LIKE 'ifood%'
       OR lower(coalesce(v_order.salesperson, '')) IN ('ifood','99food','rappi','keeta','outro')
     ) THEN
    RAISE EXCEPTION 'Pedido não é de plataforma externa (origem: %, vendedor: %)',
      coalesce(v_order.external_origin, 'null'), coalesce(v_order.salesperson, 'null');
  END IF;
