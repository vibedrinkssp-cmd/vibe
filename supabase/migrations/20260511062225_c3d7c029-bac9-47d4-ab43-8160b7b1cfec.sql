CREATE OR REPLACE FUNCTION public.insert_external_order_idempotent(
  ...
  p_address_city text DEFAULT 'São José dos Campos',
  p_address_state text DEFAULT 'SP',
...
BEGIN
  ...
  IF p_order_type::public.order_type = 'delivery' THEN
    ...
    INSERT INTO public.users (name, whatsapp, role)
    VALUES (COALESCE(NULLIF(trim(p_customer_name), ''), 'Cliente ' || p_platform), v_external_whatsapp, 'customer')
